...
The Durantic API has no path-based versioning (no `/api/v1/`) and evolves by **adding** fields and endpoints. A provider release is compatible with any control plane that carries at least the API fields its pinned client was generated from, and — from **1.1.1** onwards — with every newer control plane as well: the generated client ignores response fields it does not know about. Provider releases **before 1.1.1** (1.0.0, 1.1.0) reject unknown response fields and therefore fail against control planes newer than the API they were generated from (for 1.1.0, anything at or after controlplane `2026.08.18`); upgrade to 1.1.1 or later. Pin the provider with a `~>` version constraint as usual; any behavioral changes are called out in the [release notes](https://github.com/durantic/terraform-provider-durantic/releases).
